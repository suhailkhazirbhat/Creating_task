...
function addTask(){
    ...
    p.innerHTML=`<input type="checkbox" onchange="handleCheckbox(this)"><input  onchange="handleChange(this)" value=${taskobj.value}><button style="background-color: #272E4B; color:white; text-align: center;border: 1px solid #272E4B;border-radius: 0px 0px 14px 14px "  onclick="Delete(${taskobj.id})"> <i class="fa fa-trash" aria-hidden="true"></i></button>`
    data.push(taskobj)
    localStorage.setItem("data",JSON.stringify(data))
    task.value=''
}
function show(){
    ...
    container.appendChild(p)
    p.innerHTML=`<input type="checkbox" onchange="handleCheckbox(this)" ><input onchange="handleChange(this)" value=${element.value}><button style="background-color: #272E4B; color: white; text-align: center;border: 1px solid #272E4B;border-radius: 0px 0px 14px 14px " onclick="Delete(${element.id})"> <i class="fa fa-trash" aria-hidden="true"></i></button>`
